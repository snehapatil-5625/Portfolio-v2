            ...
            <div className="section-container">
                <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="space-y-4">
                        ...
                        <p className="text-[var(--text-secondary)] max-w-lg text-lg">
                            Selected works that showcase my technical expertise and design philosophy.
                        </p>
                    </div>
                    <button className="text-[var(--accent-primary)] font-bold flex items-center gap-2 hover:gap-3 transition-all">
                        See All Works
                        <span>→</span>
                    </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {projects.map((project, idx) => (
                        <div
                            key={idx}
                            className="group relative rounded-3xl overflow-hidden glass-effect border border-[var(--border-color)] hover:border-[var(--accent-primary)]/30 transition-all duration-500 bg-[var(--bg-card)]"
                        >
                            {/* Image Container */}
                            <div className="relative h-64 overflow-hidden">
                                <div className="absolute inset-0 bg-emerald-500/20 group-hover:bg-transparent transition-all z-10"></div>
                                <img
                                    src={project.image}
                                    alt={project.title}
                                ...
                                <div className="absolute top-4 left-4 z-20">
                                    <span className="px-3 py-1 rounded-lg bg-[var(--bg-color)]/60 backdrop-blur-md border border-[var(--border-color)] text-[var(--accent-primary)] text-xs font-bold tracking-widest uppercase">
                                        {project.category}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-8 space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2 pt-2">
                                    {project.tech.map((t) => (
                                        <span
                                            key={t}
                                            className="px-2 py-1 rounded-md bg-[var(--accent-primary)]/5 border border-[var(--border-color)] text-[10px] text-[var(--accent-primary)] uppercase font-black"
                                        >
                                            {t}
                                        </span>
                                    ))}
                                ...
                                <div className="pt-4 flex items-center justify-between">
                                    <a href="#" className="text-[var(--text-primary)] text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                                        Learn More <span className="text-[var(--accent-primary)]">→</span>
                                    </a>
